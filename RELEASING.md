...
| --- | --- |
| nuwa-build (PyPI) | `0.5.0` (`v0.5.0`) |
| nuwa-sdk (Nimble / GitHub) | `0.4.3` (already tagged) |
| nuwa-build-action | moving `v1` tag after merging Action changes |
| nuwa-docs | deploy `main` to GitHub Pages |
| nuwa-example | match template pins (`nuwa_sdk@0.4.3`, Python `>=3.10`) |
...
2. **SDK** — Confirm `v0.4.3` exists and CI is green. No version bump is required unless you change Nim sources.
...
4. **nuwa-build** — Confirm tests/lint on `main`. Changelog date is correct. Tag and push:
   ...
   PyPI publish is tag-triggered. Confirm the GitHub Release exists for `v0.5.0` (create one if the workflow does not).
5. **Example** — Merge pin and workflow updates. Do not commit `.nimble/` caches.
6. **Sanity** — On a clean machine: install Nim, `pip install nuwa-build==0.5.0`, `nuwa new demo`, `nuwa develop`, `python example.py`.
